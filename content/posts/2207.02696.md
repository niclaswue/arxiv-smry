...
UseHugoToc: false
cover:
    image: "https://ik.imagekit.io/smryai/2207-02696v1_jXjYf3ib9.jpg" # image path/url
...
- For example, multi-object tracking, autonomous driving, robotics, medical image analysis
...
- For example, the devel- opment of MCUNet [49,48] and NanoDet [54] focused on producing low-power single-chip and improving the inference speed on edge CPU
...
### Model scaling
- Model scaling is a way to make a model fit in different computing devices
...
- The proposed method is 127 fps faster and 10.7% more accurate on AP than YOLOv5-N (r6.1).
- YOLOv7 has 51.4% AP at frame rate of 161 fps, while PPYOLOE-L with the same AP has only 78 fps frame rate.
...
- In terms of the amount of parameters and computation, YOLOv7-X reduces 22% of parameters and 8% of computation compared to YOLOv5-X (r6.1), but improves AP by 2.2%.
...
- In the experiment dealing with residual-based model, since the original dark block does not have a 3...

## Conclusions
- The new architecture of realtime object detector is proposed
- The model scaling method is developed to improve the accuracy of object detection
- The replacement problem of re-parameterized module and the allocation problem of dynamic label assignment are found during the research process
- To solve the problem, a trainable bag-of-freebies method is proposed

## Acknowledgements
- The authors thank National Center for Highperformance Computing for providing computational and storage resources.
- The authors used the resources at NCHC to conduct their research.
- The authors' research was focused on using GPUs to accelerate scientific research.
- The authors' research was successful in using GPUs to accelerate scientific research.

## More comparison
- YOLOv7 surpasses all known object detectors in both speed and accuracy
- YOLOv7-E6 object detector (56 FPS V100, 55.9% AP) outperforms both transformerbased detector SWIN-L Cascade-Mask R-CNN (9.2 FPS A100, 53.9% AP) by 509% in speed and 2% in accuracy
