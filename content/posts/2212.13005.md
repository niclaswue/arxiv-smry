...
UseHugoToc: false
cover:
    image: "https://ik.imagekit.io/smryai/2212-13005v1_Jwaukrgvk.jpg" # image path/url
...
# Abstract
- TextBox 2.0 is a comprehensive library that covers text generation tasks and their corresponding datasets
- The library is easy to use, with interfaces in Python and a command line
- The library is validated through extensive experiments
...
- The new features include the ability to generate text in multiple languages, the ability to generate text with different styles, and the ability to generate text with different formats.
- The new features are useful for facilitating PLM-based text generation research.

### Generation Tasks
- 13 tasks in TextBox 2.0
- 83 datasets
- Metrics for evaluation: lexical, semantic, accuracy, and task-specific
- Visualization tools

### Generation Models
- TextBox 2.0 incorporates 45 PLMs
- These PLMs cover a wide range of tasks, such as dialogue generation, translation, and Chinese text generation
- This wide coverage makes it possible to use TextBox 2.0 for a variety of text generation tasks
...
## Library Usage
- PLMs can be run quickly and results for each dataset can be reproduced
- It is convenient to specify a dataset and model by setting the configurations dataset, model, and model_path, within a simple command line
- Pre-training a new model can be done using the pre-training objective provided
- The library provides several visualization analysis mechanisms to perform deep analysis on the generated results
...
- Reduces the time spent on non-essential functions

### Visualization Analysis
- It is important to reproduce a model to compare existing methods.
- Users can use TextBox 2.0 to conduct visualization analysis for specified models.
...
- The library makes significant extensions in three major aspects: generation tasks (13 tasks and 83 datasets), generation models (45 PLMs), and training strategies (e.g., distributed data parallel and efficient decoding)
- Results from extensive test experiments demonstrate that the library can accurately reproduce existing models
- Besides, the library provides a series of utility tools to better analyze and explore the generated results
