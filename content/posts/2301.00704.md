...
UseHugoToc: false
cover:
    image: "/home/niclas/arxiv-smry/arxiv-smry/static/thumbnails/2301-00704v1.webp" # image path/url
...
- This was enabled by a combination of deep learning architecture innovations
...
- In this work, we present a new model for text-to-image synthesis using a masked image modeling approach
- Our image decoder architecture is conditioned on embeddings from a pre-trained and frozen T5-XXL (Raffel et al., 2020) large language model (LLM) encoder
- In agreement with Imagen (Saharia et al., 2022), we find that conditioning on a pre-trained LLM is crucial for photorealistic, high quality image generation
- Our models (except for the VQGAN quantizer) are built on the Transformer (Vaswani et al., 2017) architecture
- We have trained a sequence of Muse models, ranging in size from 632M parameters to 3B parameters (for the image decoder; the T5-XXL model has an additional 4.6B parameters)
- Each model consists of several sub-models
- Muse is significantly more efficient than Imagen-3B or Parti-3B models and 3x faster than Stable Diffusion v1.4
...
- The model is able to predict the next word in a document
...
- Pre-trained large language models are beneficial to high-quality image generation
...
- The encoder has several downsampling blocks to reduce the spatial dimension of the input
- The decoder has the corresponding number of upsampling blocks to map the latents back into original image size
...
- Two VQGAN models were trained at different resolutions (16 × 16 and 256 × 256 vs 64 × 64 and 512 × 512)
- The lower resolution model is used to "translate" the higher resolution model, and the higher resolution model is used to decode the lower resolution model
- The latent map translation model is also trained with text conditioning and cross-attention
...
- The model is effective for inference of 256 tokens using only 24 decoding steps
- The model is effective for inference of 4096 tokens using 8 decoding steps
...
- An art gallery displaying Monet paintings. The art gallery is flooded. Robots are going around the art gallery using paddle boards. A photograph of the inside of a subway train. There are raccoons sitting on the seats. One of them is reading a newspaper. The window shows the city in the background. Two cups of coffee, one with latte art of yin yang symbol. The other has latter art of a heart.
...
- One of the most difficult prompts for Muse is ones with multiple cardinalities (e.g., "four cats and a team of three dogs").

### Quantitative Performance
- Table 1: Comparison of different methods on the CC3M dataset.
- Table 2: Comparison of different methods on the COCO dataset.
- The 632M model achieves SOTA results on the CC3M dataset, significantly improving upon the state of the art in FID score.
...
- Variational autoencoders and Generative Adversarial Models have been shown to be effective at generating images
