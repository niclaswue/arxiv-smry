...
UseHugoToc: false
cover:
    image: "/home/niclas/arxiv-smry/arxiv-smry/static/thumbnails/2301-02538v1.webp" # image path/url
...
- Each split used 75% for training and 25% for testing. Ten percent of the training images were used for validation, where the dice similarity coefficient (DSC) was used to pick the best model state (every 50 epochs).
...
- A case with 10 or more annotated cysts was considered complex. This gave a total of 65 easy cases and 52 complex cases.
- When evaluating DSC, all tumors were treated as one mask. This can cause bias toward the DSC of larger tumors.
...
- Then we measured the DSC of overlapping connected components to get the DSC for each ground truth tumor. These were averaged per patient and then averaged over all patients to get the average per-tumor DSC.
- In addition to DSC, AUC of ccRCC tumor detection was also evaluated.
- For a given probability threshold, a tumor was considered detected if the 90 th percentile of the probabilities inside the tumor exceeded the threshold. This means that at least 10% of the tumors had probability scores exceeding the threshold.
...
- All experiments were run multiple times
- All experiments were run with different input data
- All experiments were run to produce the same output
- All experiments were run on a Linux (CentOS 7)
- All experiments were run with the same input data
- All experiments were run multiple times
- All experiments were run with different input data
...
- The model may not work as well with other MR sequences
...
- 115 images with 2 patients having both kidneys annotated were included
- 504 lesions were found on the annotated images

## Unnamed Section 515
- A two-dimensional U-network is used to segment and analyze images.
- The workflow begins with image acquisition and continues with segmentation and co-registration.

## Unnamed Section 516
- A graph is a visual representation of data.
- A graph can be used to show relationships between data.
- A graph can be used to show how data changes over time.
- A graph can be used to show how data is distributed.
- A graph can be used to show how data is related.

## Unnamed Section 517
- Numerous unannotated cysts were found in the ground truth mask
- These cysts may be indicative of a data leak
- Further investigation is needed to determine the cause of the leak

## Unnamed Section 518
- The computer has an Intel Xeon Gold 6140 processor and NVIDIA Tesla V100x GPU
- The computer is running Python 3.8 and PyTorch 1.10 configured for CUDA 10.2 and cuDNN 7.6

## Unnamed Section 519
- The best model performed well on the U-Net test set segmentations for kidney, cyst and tumor.
- The worst model performed poorly on the U-Net test set segmentations for kidney, cyst and tumor.

## Unnamed Section 520
- The voxel percent confusion matrix and box plot illustrate the misclassification of each label in the 2D U-net.
- Tumor pixels are mostly detected as kidney or background falsely.

## Unnamed Section 521
- The Tumor Detection ROC curve shows the rate of detection for a given false positive rate.
- The yellow and blue shaded regions reflect two standard deviations of detection rate for a given false positive rate.

## Unnamed Section 522

## Unnamed Section 523
- The DSC score for kidney, cyst and/or tumor can suffer if unannotated structures are properly segmented
- To overcome unlabeled cysts during training, T1 hypointense regions that are labeled kidney are relabeled with an "ignore label."
- Consequently, any benign renal structure that is neither tumor nor cyst will not be learned and might harm model's generalizability.
- Due to GPU memory constraints, the 2D U-Net can access more global image context while the 3D U-Net is more localized.
